...
def format_product(product_row):
    return {
        ...
        "entry_date": product_row[3].isoformat() if product_row[3] else None,
        "stock": product_row[4]
    }
...
def format_product_detail(product):
    if isinstance(product, dict):
        return {
            ...
            "entry_date": product["entry_date"].isoformat() if product["entry_date"] else None,
            "stock": product["stock"]
        }
    # If its a tuple list
...
def cache_invalidation(cache_manager, product_id):
